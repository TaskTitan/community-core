...
class ParallelExecution extends BaseAction {
  ...
  async execute(params, inputData, workflowEngine) {
    ...
    const executeNodeAndConnected = async (nodeName, inputData) => {
      try {
        const taskNode = workflowEngine.workflow.nodes.find(
          (n) =>
            n.text.toLowerCase().replace(/\s+/g, "") ===
            nodeName.toLowerCase().replace(/\s+/g, "")
        );
        if (!taskNode) {
        ...
        const connectedEdges = workflowEngine.workflow.edges.filter(
          (edge) => edge.start.id === taskNode.id
        );
        const connectedResults = await Promise.all(
          connectedEdges.map(async (edge) => {
            if (workflowEngine.edgeEvaluator.evaluateEdgeCondition(edge, result)) {
              ...
              const connectedNode = workflowEngine.workflow.nodes.find(
                (n) => n.id === edge.end.id
              );
    ...
    const tasks = nodeNames.map((nodeName) =>
      executeNodeAndConnected(nodeName, inputData)
    );
